...
Thus, we will use a non-parametric test. For the precise test to be used, we can see that the samples are paired (before and after treatment for the same subjects), so we will use the Wilcoxon signed-rank test that is designed for paired samples.

```{r wilcoxon-test }
# Create two new variables for before and after treatment for 8 subjects
data$perf_pre <- data$perf[1:8]
data$perf_post <- data$perf[9:16]
data <- data[1:8, ]  # Keep only the first 8 rows for analysis
data$perf_pre
data$perf_post

wilcox.test(data$perf_pre, data$perf_post, paired = TRUE, alternative = "two.sided")
```
...
median_pre <- median(data$perf_pre)
median_post <- median(data$perf_post)
