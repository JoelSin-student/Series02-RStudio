...
```{r load-prepost-data, message=FALSE, warning=FALSE}
data <- read.csv("data/test_data/PrePost.csv", sep = ",", header=TRUE)
summary(data)
```
...
We will decide it over the results of our statistical analysis.
...
We can observe that the sample size is small (n=8), which makes difficult to assess the normality of the distribution. Thus, we will use a non-parametric test. For the precise test to be used, we can see that the samples are paired (before and after treatment for the same subjects), so we will use the Wilcoxon signed-rank test that is designed for paired samples.
